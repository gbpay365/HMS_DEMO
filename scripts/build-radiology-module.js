/**
 * One-shot: generate radTestTemplates.js, radApi.js, radiology-templates assets from lab counterparts.
 */
const fs = require('fs');
const path = require('path');
const root = path.join(__dirname, '..');

// ── 1. radTestTemplates.js ─────────────────────────────────────────────
const radTemplateSrc = `/**
...
function suggestTemplateForOrderName(orderName) {
  ...
  if (best) return { catKey: best.categoryKey, testId: best.id, testName: best.name };
  return null;
}
...
fs.writeFileSync(path.join(root, 'lib', 'radTestTemplates.js'), radTemplateSrc, 'utf8');
console.log('wrote lib/radTestTemplates.js');

// ── 2. radApi.js from labApi.js ───────────────────────────────────────
let labApi = fs.readFileSync(path.join(root, 'routes', 'labApi.js'), 'utf8');
const radApi = labApi
  .replace(/labTestTemplates/g, 'radTestTemplates')
  .replace(/LAB_TEST_TEMPLATES/g, 'RAD_TEST_TEMPLATES')
  .replace(/createLabApi/g, 'createRadApi')
  .replace(/\/api\/lab/g, '/api/rad')
  .replace(/laboratory/g, 'radiology')
  .replace(/Laboratory/g, 'Radiology')
  .replace(/tbl_lab_result/g, 'tbl_radiology_result')
  .replace(/lab_result_id/g, 'radiology_result_id')
  .replace(/labResultId/g, 'radiologyResultId')
  .replace(/lab_result/g, 'radiology_result')
  .replace(/Lab /g, 'Radiology ')
  .replace(/lab /g, 'rad ')
  .replace(/buildLabInsertRow/g, 'buildRadInsertRow')
  .replace(/labResultColumns/g, 'radResultColumns')
  .replace(/formatNotesFromReport/g, 'formatFindingsFromReport')
  .replace(/module: 'laboratory'/g, "module: 'radiology'")
  .replace(/prefix !== 'LAB'/g, "prefix !== 'RAD'")
  .replace(/startsWith\('LAB-'\)/g, "startsWith('RAD-')")
  .replace(/item_type = 'laboratory'/g, "item_type = 'radiology'")
  .replace(/item_type !== 'laboratory'/g, "item_type !== 'radiology'")
  .replace(/'laboratory order item'/g, "'radiology order item'")
  .replace(/lab registry/g, 'radiology registry')
  .replace(/lab standalone/g, 'radiology standalone')
  .replace(/lab order-context/g, 'rad order-context')
  .replace(/lab alert-context/g, 'rad alert-context')
  .replace(/lab registry-result-context/g, 'rad registry-result-context')
  .replace(/lab report\/persist/g, 'rad report/persist')
  .replace(/radiology_result_id: null/g, 'radiology_result_id: null')
  .replace(/lab_result_id: ex.id/g, 'lab_result_id: null');

// Fix persist field mapping: notes -> findings, test_name -> exam_name
const radApiFixed = radApi
  .replace(/function formatFindingsFromReport\(rep\) \{[\s\S]*?return lines\.join\('\\n'\);\n\}/,
...
fs.writeFileSync(path.join(root, 'routes', 'radApi.js'), radApiFixed, 'utf8');
console.log('wrote routes/radApi.js');

// ── 3. radiology-templates.js from laboratory-templates.js ────────────
let labJs = fs.readFileSync(path.join(root, 'public', 'js', 'laboratory-templates.js'), 'utf8');
labJs = labJs
  .replace(/laboratory-templates/g, 'radiology-templates')
  .replace(/loadLabTemplates/g, 'loadRadTemplates')
  .replace(/LAB_TEST_TEMPLATES/g, 'RAD_TEST_TEMPLATES')
  .replace(/__LAB_ORDER/g, '__RAD_ORDER')
  .replace(/\/api\/lab\//g, '/api/rad/')
  .replace(/labTpl/g, 'radTpl')
  .replace(/LabTpl/g, 'RadTpl')
  .replace(/clearLabTplForm/g, 'clearRadTplForm')
  .replace(/saveLabTplReport/g, 'saveRadTplReport')
  .replace(/laboratory/g, 'radiology')
  .replace(/Laboratory/g, 'Radiology')
  .replace(/lab_result_id/g, 'radiology_result_id')
  .replace(/labResultId/g, 'radiologyResultId')
  .replace(/Structured test templates/g, 'Structured exam templates')
  .replace(/test library/g, 'exam library')
  .replace(/fa-flask/g, 'fa-film')
  .replace(/fa-medkit/g, 'fa-picture-o');
fs.writeFileSync(path.join(root, 'public', 'js', 'radiology-templates.js'), labJs, 'utf8');
console.log('wrote public/js/radiology-templates.js');

// ── 4. CSS ────────────────────────────────────────────────────────────
let css = fs.readFileSync(path.join(root, 'public', 'css', 'laboratory-templates.css'), 'utf8');
css = css.replace(/labtpl/g, 'radtpl').replace(/#0d9488/g, '#7c3aed');
fs.writeFileSync(path.join(root, 'public', 'css', 'radiology-templates.css'), css, 'utf8');
console.log('wrote public/css/radiology-templates.css');

// ── 5. EJS view ───────────────────────────────────────────────────────
let ejs = fs.readFileSync(path.join(root, 'views', 'laboratory-templates.ejs'), 'utf8');
ejs = ejs
  .replace(/laboratory-templates\.css/g, 'radiology-templates.css')
  .replace(/laboratory-templates\.js/g, 'radiology-templates.js')
  .replace(/labTpl/g, 'radTpl')
  .replace(/\/laboratory/g, '/radiology')
  .replace(/Laboratory registry/g, 'Radiology registry')
  .replace(/Structured test templates/g, 'Structured exam templates')
  .replace(/test library/g, 'exam library')
  .replace(/fa-flask/g, 'fa-film')
  .replace(/fa-medkit/g, 'fa-picture-o')
  .replace(/laboratory registry/g, 'radiology registry')
  .replace(/Lab templates/g, 'Radiology templates');
fs.writeFileSync(path.join(root, 'views', 'radiology-templates.ejs'), ejs, 'utf8');
console.log('wrote views/radiology-templates.ejs');

// ── 6. radiology-report-view.ejs ──────────────────────────────────────
let repEjs = fs.readFileSync(path.join(root, 'views', 'laboratory-report-view.ejs'), 'utf8');
repEjs = repEjs
  .replace(/\/laboratory/g, '/radiology')
  .replace(/Laboratory/g, 'Radiology')
  .replace(/Lab test/g, 'Imaging exam')
  .replace(/#LAB-/g, '#RAD-')
  .replace(/fa-flask/g, 'fa-film')
  .replace(/Lab templates/g, 'Radiology templates')
  .replace(/row\.test_name/g, 'row.exam_name || row.test_name');
fs.writeFileSync(path.join(root, 'views', 'radiology-report-view.ejs'), repEjs, 'utf8');
console.log('wrote views/radiology-report-view.ejs');

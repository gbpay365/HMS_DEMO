...
/**
 * Write DOCTOR-USERS-MANUAL.html and .pdf to docs/
 * Usage: node scripts/generate-doctor-manual-pdf.js
 */

const fs = require('fs');
const path = require('path');
const { buildDoctorUsersManualHtml } = require('../lib/hmsDoctorUserManual');
const { htmlToPdfBuffer } = require('../lib/passportPdf');

const docsDir = path.join(__dirname, '..', 'docs');
const htmlPath = path.join(docsDir, 'DOCTOR-USERS-MANUAL.html');
const pdfPath = path.join(docsDir, 'DOCTOR-USERS-MANUAL.pdf');

const html = buildDoctorUsersManualHtml();
let outHtml = html;

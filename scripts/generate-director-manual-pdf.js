...
/**
 * Write DIRECTOR-USERS-MANUAL.html and .pdf to docs/
 * Usage: node scripts/generate-director-manual-pdf.js
 */

const fs = require('fs');
const path = require('path');
const { buildDirectorUsersManualHtml } = require('../lib/hmsDirectorUserManual');
const { htmlToPdfBuffer } = require('../lib/passportPdf');

const docsDir = path.join(__dirname, '..', 'docs');
const htmlPath = path.join(docsDir, 'DIRECTOR-USERS-MANUAL.html');
const pdfPath = path.join(docsDir, 'DIRECTOR-USERS-MANUAL.pdf');

const html = buildDirectorUsersManualHtml();
let outHtml = html;

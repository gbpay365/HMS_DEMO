...
const PAYOUT_GL_ACCOUNT = '421100';
...
function mapGlAccounts(txnType, paymentMethod, serviceKey, expenseCategory) {
  ...
  if (t === 'payout') {
    return { gl_debit_account: PAYOUT_GL_ACCOUNT, gl_credit_account: cashGl.code };
  }
  if (t === 'refund') {
...
async function syncDisbursementExpenseJournal(pool, opts) {
  const { journalPostExtended, cashLikeAccount } = require('./hmsFinJournalPost');
  const { buildExpenseLines, loadTvaRate } = require('./finPostingTemplates');
  ...
  const category = opts.expenseCategory || 'general';
  const glLines = buildExpenseLines(amt, opts.paymentMethod || 'Cash', cashLikeAccount, category, tvaRate, APPLY_TVA_ON_SYNC);
  const label = `${opts.cashierCode || ''} ${opts.cashierIdentity || ''}`.trim();
...
async function syncPayoutJournalAfterCashier(pool, opts) {
  const { journalPostExtended } = require('./hmsFinJournalPost');
  const facilityId = parseInt(String(opts.facilityId || 1), 10) || 1;
  const payoutId = parseInt(String(opts.expenseId || opts.txnId || 0), 10) || 0;
  const amt = roundMoney(opts.amount);
  if (payoutId < 1 || amt <= 0) return { ok: false, journalHeaderId: null };
  ...
  const r = await journalPostExtended(pool, {
    facilityId,
    sourceType: 'cashier_payout',
    sourceId: payoutId,
    reference: String(opts.reference || `PO-${payoutId}`).slice(0, 64),
    narration: `Cashier payout · ${label} · ${String(opts.narration || '').slice(0, 400)}`,
    createdBy: opts.createdBy || 0,
    lines: [
      { code: PAYOUT_GL_ACCOUNT, label: 'Admin / emergency payout', debit: amt, credit: 0 },
      { code: cashGl.code, label: cashGl.label, debit: 0, credit: amt },
    ],
    journalCode: 'ACH',
    status: 'posted',
  ...
  const journalHeaderId = r.journalId || (await findJournalHeaderId(pool, facilityId, 'cashier_payout', payoutId));
  if (opts.txnId && journalHeaderId) await linkCashierTxnJournal(pool, opts.txnId, journalHeaderId);
  return { ok: r.ok || r.duplicate, journalHeaderId, duplicate: r.duplicate };
}

const CASHIER_GL_SOURCE_TYPES = new Set([
  'billing_receipt',
  'cashier_refund',
  'cashier_payout',
...
function isCashierGlSource(sourceType) {
  return CASHIER_GL_SOURCE_TYPES.has(String(sourceType || '').trim());
}

/**
 * After DB commit: post journal + push to Account_Core (receipt webhook or GL journal).
 */
async function afterCommitCashierPipeline(pool, opts) {
  ...
  if (opts.journalHeaderId) {
    journal = { ok: true, journalHeaderId: opts.journalHeaderId, duplicate: false };
  ...
  const journalId = parseInt(String(journal?.journalHeaderId || opts.journalHeaderId || 0), 10) || 0;
  let external = null;

  // Patient receipts: prefer cashier receipt webhook (matches Core cashier_receipt ext ref).
  if (opts?.txnId && kind === 'receipt') {
    try {
      const { syncCashierTxnToCoreAccount } = require('./coreAccountWebhook');
      external = await syncCashierTxnToCoreAccount(pool, opts.txnId);
    } catch (e) {
      external = { ok: false, error: e.message || String(e) };
    }
  } else if (opts?.txnId && !(journal?.ok && journalId)) {
    try {
      const { syncCashierTxnToCoreAccount } = require('./coreAccountWebhook');
      external = await syncCashierTxnToCoreAccount(pool, opts.txnId);
    } catch (e) {
  ...
  // Fallback / primary when receipt webhook unavailable: push full HMS GL journal.
  if (journalId > 0 && !external?.ok) {
    try {
      const { pushJournalSyncToAccountCore } = require('./journalAccountCoreSync');
      await pushJournalSyncToAccountCore(pool, journalId, facilityId);
    } catch (e) {
      console.warn('[journal-core-sync] cashier pipeline', journalId, e.message || e);
    }
  }

...
const { withSavepoint } = require('./pgTransaction');
...
function isCashierTxnSourceConflict(err) {
  const msg = String(err?.message || err || '');
  return /uq_cashier_txn_source|duplicate key.*source_module/i.test(msg);
}

async function insertCashierTxnRow(db, params, meta = {}) {
  ...
  const handleConflict = async (err) => {
    if (isCashierTxnSourceConflict(err)) {
      const existing = await findExistingCashierTxn(db, meta.sourceModule, meta.sourcePk);
      if (existing) return [{ insertId: existing.id }];
    }
    if (db?.driver === 'postgres' && isCashierTxnPrimaryKeyConflict(err)) {
      await syncCashierTxnIdSequence(db);
      return withSavepoint(db, 'cashier_txn_ins_retry', () => db.query(sql, params));
    }
    throw err;
  };
  ...
  try {
    return await withSavepoint(db, 'cashier_txn_ins', runInsert);
  } catch (err) {
    return handleConflict(err);
  }
}
...
async function recordCashierTransaction(db, ctx) {
  ...
  let cashier = ctx.precachedCashier || null;
  if (!cashier) {
    cashier = await resolveCashierForEmployee(db, employeeId, {
      facilityId,
  ...
  const [ins] = await insertCashierTxnRow(
    db,
    [
      facilityId,
      cashier.id,

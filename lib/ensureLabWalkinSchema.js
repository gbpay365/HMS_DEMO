...
/** Lab walk-in queue — lightweight registration before cashier payment. */
...
module.exports = async function ensureLabWalkinSchema(pool) {
  if (!pool || !pool.query) return;

  if (pool.driver === 'postgres') {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS tbl_lab_walkin (
        ...
        lab_request_id INTEGER DEFAULT NULL,
    ...
    `).catch(() => {});
    await pool
      .query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_lab_walkin_reg_no ON tbl_lab_walkin (registration_no)`)
      .catch(() => {});
    await pool
      .query(`CREATE INDEX IF NOT EXISTS idx_lab_walkin_status ON tbl_lab_walkin (status)`)
      .catch(() => {});
    await pool
      .query(`CREATE INDEX IF NOT EXISTS idx_lab_walkin_created ON tbl_lab_walkin (created_at)`)
      .catch(() => {});

    await pool.query(`
      CREATE TABLE IF NOT EXISTS tbl_lab_walkin_line (
        id SERIAL PRIMARY KEY,
    ...
    `).catch(() => {});
    await pool
      .query(`CREATE INDEX IF NOT EXISTS idx_lab_walkin_line_walkin ON tbl_lab_walkin_line (walkin_id)`)
      .catch(() => {});

...
async function resolvePatientId(pool, input, { facilityId = 1 } = {}) {
  const billingCompanyId = parseInt(String(input.billing_company_id), 10) || 0;
  if (billingCompanyId > 0) {
...
async function createCashierInvoice(pool, input, session) {
  ...
  let billToName = String(input.bill_to_name || '').trim().slice(0, 200);
  let billToCompany = String(input.bill_to_company || '').trim().slice(0, 2000);
  let billToContact = String(input.bill_to_contact || '').trim().slice(0, 200);
  const billingCompanyId = parseInt(String(input.billing_company_id), 10) || 0;
  ...
  if (billingCompanyId > 0) {
    const company = await getBillingCompanyById(pool, billingCompanyId, fid);
    if (!company) {
      return { ok: false, error: 'Selected company was not found.', status: 400 };
    }
    billToName = company.name;
    billToCompany = formatCompanyBillingBlock(company);
    if (!billToContact) billToContact = primaryContact(company);
  }

  if (billingCompanyId < 1 && patientIdEarly < 1 && !billToCompany) {
    return {
      ok: false,
      error: 'Select a patient or choose a company for billing.',
      status: 400,
    };
  }
  if (billingCompanyId < 1 && patientIdEarly < 1 && billToCompany) {
    return {
      ok: false,
      error: 'Select a company from the list or add a new company profile.',
      status: 400,
    };
  }
  ...
  const invoiceNumber = await nextInvoiceNumber(pool, fid);
  ...
  const [ins] = await pool.query(
    `INSERT INTO tbl_payment_ticket
     (facility_id, ticket_code, patient_id, status, total_amount, amount_paid, lines_json,
      ...
      billing_company_id, issue_date, due_date, discount_pct, tax_pct, invoice_status, invoice_number, created_by, created_at)
     VALUES (?, ?, ?, 'pending', ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      fid,
      ticketCode,
      ...
      billToContact || null,
      billToCompany || null,
      billingCompanyId > 0 ? billingCompanyId : null,
      issueDate,
      dueDate,
      discountPct,
      taxPct,

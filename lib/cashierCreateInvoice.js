...
async function resolvePatientId(pool, input) {
  ...
  const like = `%${billTo.replace(/[%_\\]/g, ' ').trim()}%`;
  const [rows] = await pool
    .query(
      `SELECT id FROM tbl_patient
       WHERE status = 1
         AND LOWER(CONCAT(COALESCE(first_name,''), ' ', COALESCE(last_name,''))) LIKE LOWER(?)
       ORDER BY id DESC
       LIMIT 1`,
      [like]
    )
...
async function createCashierInvoice(pool, input, session) {
  ...
  if (patientId < 1) {
    return {
      ok: false,
      error: 'Select a registered patient from suggestions, or register the company/patient first.',
      status: 400,
    };
  }

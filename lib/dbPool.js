...
function normalizePostgresConnectionConfig(config) {
  if (!config || !config.connectionString) return config;
  if (config.ssl !== false) return config;
  try {
...
function createPostgresPool(config) {
  const cfg = normalizePostgresConnectionConfig(config);
  const ssl = cfg.ssl === false ? false : cfg.ssl ? cfg.ssl : false;
  const pool = new PgPool({
    connectionString: cfg.connectionString,
    ssl,
    max: parseInt(process.env.DB_POOL_SIZE || '10', 10),
    idleTimeoutMillis: 30000,
    connectionTimeoutMillis: parseInt(process.env.PG_CONNECT_TIMEOUT_MS || '20000', 10),
    keepAlive: true,
...
module.exports = { createDbPool, resolveDbConfig, normalizePostgresConnectionConfig };

...
async function postReceiptJournal(pool, opts) {
  ...
  if (!journal?.ok) {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7824/ingest/7799ec2f-1013-4dae-a65a-dcfd2e3f62ad',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'968473'},body:JSON.stringify({sessionId:'968473',location:'cashierTxnWire.js:postReceiptJournal',message:'journal post failed',data:{billingDocumentId,grandTotal,error:err||'unknown',journal},timestamp:Date.now(),hypothesisId:'E',runId:'post-fix'})}).catch(()=>{});
    // #endregion
    return { ...journal, error: err || 'journal post failed' };
  }
  // #region agent log
  fetch('http://127.0.0.1:7824/ingest/7799ec2f-1013-4dae-a65a-dcfd2e3f62ad',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'968473'},body:JSON.stringify({sessionId:'968473',location:'cashierTxnWire.js:postReceiptJournal',message:'journal post ok',data:{billingDocumentId,journalHeaderId:journal.journalHeaderId,journalCode:journal.journalCode},timestamp:Date.now(),hypothesisId:'E',runId:'post-fix'})}).catch(()=>{});
  // #endregion
  return journal;
}
...
async function runCashierPostCommit(pool, opts) {
  try {
    ...
    if (kind === 'receipt' && !opts?.txnId && opts?.billingDocumentId) {
      ...
      if (journal?.journalHeaderId) {
        try {
          const { pushJournalSyncToAccountCore } = require('./journalAccountCoreSync');
          await pushJournalSyncToAccountCore(pool, journal.journalHeaderId, opts.facilityId || 1);
        } catch (e) {
          console.warn('[journal-core-sync] receipt fallback', journal.journalHeaderId, e.message);
        }
      }

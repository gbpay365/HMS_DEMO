 ...
 */
const name = String(process.env.HMS_BRAND_NAME || 'Nigeria HMS').trim() || 'Nigeria HMS';
const tagline = String(process.env.HMS_BRAND_TAGLINE || 'Healthcare for Nigeria').trim();
const legalName = String(process.env.HMS_FACILITY_LEGAL_NAME || 'Nigeria HMS').trim();
const facilityName = String(process.env.HMS_FACILITY_NAME || 'Nigeria HMS Hospital').trim();
...
const markPath = String(process.env.HMS_MARK_PATH || '/img/zaizens-mark.svg').trim();
const faviconPath = String(process.env.HMS_FAVICON_PATH || '/img/zaizens-favicon.svg').trim();
const letterheadPath = String(process.env.HMS_LETTERHEAD_PATH || '').trim();

...
async function ensurePortalSchema(pool) {
  ...
  await pool.query(
    `UPDATE tbl_acl_portal SET home_url='/cashier?page=dashboard', is_builtin=1 WHERE code='cashier'`
  ).catch((e) => console.warn('[ensurePortalSchema] cashier home_url force', e.message));

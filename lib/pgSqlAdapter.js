...
const PG_IDENT_TOKEN = '\u0000PGID';

function stashBacktickIdentifiers(sql) {
  const placeholders = [];
  const text = String(sql).replace(/`([^`]+)`/g, (_, name) => {
    const idx = placeholders.length;
    placeholders.push(`"${name}"`);
    return `${PG_IDENT_TOKEN}${idx}${PG_IDENT_TOKEN}`;
  });
  return { text, placeholders };
}

function restorePgIdentifiers(text, placeholders) {
  let out = String(text);
  placeholders.forEach((ident, i) => {
    out = out.split(`${PG_IDENT_TOKEN}${i}${PG_IDENT_TOKEN}`).join(ident);
  });
  return out;
}
...
function translateMysqlStringLiterals(sql) {
  ...
  return String(sql).replace(/"([^"]*)"/g, (_match, content) => {
...
function translateIfFunction(sql) {
  ...
  while (i < len) {
    const m = text.slice(i).match(/\bIF\s*\(/i);
    if (!m || m.index == null) {
      out += text.slice(i);
      break;
    }
    ...
    while (j < len && depth > 0) {
      ...
      else if (ch === ',' && depth === 1) commaAt.push(j);
      j += 1;
    }
    if (commaAt.length < 2) {
    ...
    const cond = text.slice(open, commaAt[0]).trim();
    const thenV = text.slice(commaAt[0] + 1, commaAt[1]).trim();
    const elseV = text.slice(commaAt[1] + 1, j - 1).trim();
    out += `(CASE WHEN ${cond} THEN ${thenV} ELSE ${elseV} END)`;
    i = j;
  }
  return out;
}
...
function translateDateSub(sql) {
  return String(sql).replace(
    /\bDATE_SUB\s*\(\s*((?:\?|\$\d+|(?:[^()]|\([^()]*\))+))\s*,\s*INTERVAL\s+(\?|\$\d+|\d+)\s+(DAY|DAYS|HOUR|HOURS|MINUTE|MINUTES|SECOND|SECONDS|MONTH|MONTHS|YEAR|YEARS)\s*\)/gi,
    (_match, expr, amount, unit) => {
      ...
      if (amount === '?' || /^\$\d+$/.test(amount)) {
        return `(${base} - (${amount}::text || ' ${pgUnit}')::interval)`;
      }
      return `(${base} - INTERVAL '${amount} ${pgUnit}')`;
    }
  );
}

function translateDateAdd(sql) {
  return String(sql).replace(
    /\bDATE_ADD\s*\(\s*((?:\?|\$\d+|(?:[^()]|\([^()]*\))+))\s*,\s*INTERVAL\s+(\?|\$\d+|\d+)\s+(DAY|DAYS|HOUR|HOURS|MINUTE|MINUTES|SECOND|SECONDS|MONTH|MONTHS|YEAR|YEARS)\s*\)/gi,
    (_match, expr, amount, unit) => {
      const u = String(unit || 'day')
        .toLowerCase()
        .replace(/s$/, '');
...
function adaptMysqlSqlToPg(sql) {
  let text = String(sql || '');
  const { text: sqlWithIdTokens, placeholders: pgIdPlaceholders } = stashBacktickIdentifiers(text);
  text = sqlWithIdTokens;
  ...
  text = translateDateSub(text);
  text = translateDateAdd(text);
  ...
  // Text search: PostgreSQL LIKE is case-sensitive; ILIKE matches MySQL CI collations.
  text = text.replace(/\bNOT LIKE\b/gi, 'NOT ILIKE');
  ...
  // ON DUPLICATE KEY UPDATE -> ON CONFLICT DO UPDATE (composite PK when UPDATE only touches non-key cols)
  const dupMatch = text.match(
    /^(INSERT\s+INTO\s+("?[a-zA-Z0-9_]+"?)\s*\(([^)]+)\)\s*VALUES\s*\(([\s\S]+?)\)\s*)ON\s+DUPLICATE\s+KEY\s+UPDATE\s+([\s\S]+)$/i
  );
  if (dupMatch) {
    const cols = dupMatch[3]
      ...
      .filter(Boolean);
    const updates = dupMatch[5].trim().replace(/;\s*$/, '');
    const updateCols = updates
      .split(',')
      .map((part) => part.split('=')[0].trim().replace(/^"|"$/g, ''))
      .filter(Boolean);
    const conflictCols = cols.filter((c) => !updateCols.includes(c));
    const conflictTarget =
      conflictCols.length > 0
        ? conflictCols.map((c) => `"${c}"`).join(', ')
        : `"${cols[0] || 'id'}"`;
    text = `${dupMatch[1]}ON CONFLICT (${conflictTarget}) DO UPDATE SET ${updates}`;
  }
  ...
  text = text.replace(/\bTINYINT\s*\(\s*\d+\s*\)/gi, 'SMALLINT');
  text = text.replace(/\bTINYINT\b/gi, 'SMALLINT');
  text = text.replace(/\bSMALLINT\s*\(\s*\d+\s*\)/gi, 'SMALLINT');
  ...
  return restorePgIdentifiers(text, pgIdPlaceholders);
}
...
function buildInsertReturning(sql) {
  ...
  // Many HMS tables use composite keys (e.g. tbl_receipt_seq) — never assume an "id" column.
...
function toMysqlStyleResult(sql, pgResult) {
  ...
  if (pgResult.rows && pgResult.rows.length) {
    ...
    else if (row && row.document_id != null) header.insertId = Number(row.document_id) || 0;

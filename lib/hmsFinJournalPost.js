...
async function maybeReplicateJournalToCoreAccount(pool, header) {
  const { ensureIntegrationSchema } = require('./ensureIntegrationSchema');
  const { syncOneJournalToAccountCore } = require('./syncJournalsToAccountCore');
  const fid = parseInt(String(header.facility_id || cfg.facilityId()), 10) || 1;
  const s = await getSettings(pool, fid);
  const enabled = !!(s.core_account_sync_enabled || cfg.isIntegrationEnabled());
  // #region agent log
  fetch('http://127.0.0.1:7824/ingest/7799ec2f-1013-4dae-a65a-dcfd2e3f62ad',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'968473'},body:JSON.stringify({sessionId:'968473',location:'hmsFinJournalPost.js:maybeReplicate',message:'journal replicate hook',data:{journalId:header.id,enabled,urlHost:(()=>{try{return new URL(s.core_account_url||cfg.coreAccountUrl()).host;}catch(_){return null;}})()},timestamp:Date.now(),hypothesisId:'H',runId:'post-fix'})}).catch(()=>{});
  // #endregion
  if (!enabled) return null;
  await ensureIntegrationSchema(pool);
  return syncOneJournalToAccountCore(pool, header);
}
...
async function journalPostExtended(pool, opts) {
  ...
  const lineCodes = (lines || []).map((ln) => String(ln.code || '').trim()).filter(Boolean);
  if (lineCodes.length) {
    const { ensureFinAccountsForCodes } = require('./finAccountSeedData');
    const ensured = await ensureFinAccountsForCodes(pool, lineCodes, { facilityId: fid }).catch(() => null);
    // #region agent log
    fetch('http://127.0.0.1:7824/ingest/7799ec2f-1013-4dae-a65a-dcfd2e3f62ad',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'968473'},body:JSON.stringify({sessionId:'968473',location:'hmsFinJournalPost.js:journalPostExtended',message:'ensure posting accounts',data:{sourceType:st,sourceId:sid,lineCodes,ensured},timestamp:Date.now(),hypothesisId:'E',runId:'post-fix'})}).catch(()=>{});
    // #endregion
  }
  ...
  try {
    ...
    if (postStatus === 'posted' && jid > 0) {
      await maybeReplicateJournalToCoreAccount(pool, {
        id: jid,
        facility_id: fid,
        entry_date: ed,
        reference: ref,

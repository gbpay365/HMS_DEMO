...
const { resolveJournalEntryLines, defaultAutoCreateSubAccounts } = require('./finCoaSubAccount');
...
async function journalPostExtended(pool, opts) {
  ...
  const {
    ...
    reversalOfId = null,
    pieceNumber = null,
    autoCreateSubAccounts = undefined,
  } = opts || {};
  ...
  const autoCreate = defaultAutoCreateSubAccounts(st, autoCreateSubAccounts);
  const resolvedLines = await resolveJournalEntryLines(pool, lines, {
    facilityId: fid,
    autoCreate,
  });
  if (!resolvedLines.ok) {
    const err = resolvedLines.error || "Can't Post on Main Account , Please Create Sub Account and then Post the Transaction";
    journalPostSetLastError(`validation: ${err}`);
    return {
      ok: false,
      journalId: 0,
      duplicate: false,
      code: 0,
      needsSubAccounts: Boolean(resolvedLines.needsSubAccounts),
      missing: resolvedLines.missing || [],
      error: err,
      createdAccounts: resolvedLines.created || [],
    };
  }
  const linesToValidate = resolvedLines.lines;
  ...
  try {
    ...
    for (const ln of normLines) {
      ...
      const ccId = ln.cost_center_id != null ? parseInt(ln.cost_center_id, 10) || null : null;
      await conn.query(
        `INSERT INTO tbl_fin_journal_line
         (journal_id, account_code, account_label, debit, credit, line_memo, tier_id, tva_rate, tva_amount, cost_center_id)
         VALUES (?,?,?,?,?,?,?,?,?,?)`,
        [jid, code, lab, dr, cr, memo, tierId, tvaRate, tvaAmt, ccId]
      );
      lineInserts++;
    }
    ...
    if (postStatus === 'posted') {
      ...
      if (st !== 'account_core') {
        const { scheduleJournalSyncToAccountCore } = require('./journalAccountCoreSync');
        scheduleJournalSyncToAccountCore(pool, jid, fid);
      }
    } else {
...
async function postCreditPaymentCollection(
 ...
 return journalPostExtended(pool, {
  ...
  lines: [
   ...
   { code: glMaps().receivable, label: glMaps().receivableLabel, debit: 0, credit: amt },
  ],
  entryDate,
  journalCode: 'VTE',
  status: 'posted',
...
async function journalPostManualWithResult(pool, facilityId, entryDate, reference, narration, createdBy, lines, options = {}) {
 ...
 const glLines = lines.map((ln) => ({
  ...
  cost_center_id: ln.cost_center_id != null ? ln.cost_center_id : null,
 }));
 ...
 const r = await journalPostExtended(pool, {
  ...
  autoCreateSubAccounts: options.autoCreateSubAccounts,
 });

 if (r.needsSubAccounts) {
  return {
    ok: false,
    journalId: 0,
    duplicate: false,
    ...
    missing: r.missing || [],
    error: r.error || journalPostLastError(),
  };
 }
 if (r.ok && (r.code === 1 || r.duplicate)) {
  ...
  return { ok: true, journalId, duplicate: r.duplicate, error: '', createdAccounts: r.createdAccounts || [] };
 }

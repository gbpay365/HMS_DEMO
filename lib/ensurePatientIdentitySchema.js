...
async function ensurePatientIdentitySchema(pool) {
  ...
  for (const row of rows || []) {
    ...
    if (!key) continue;
    await pool
      .query('UPDATE tbl_patient SET patient_identity_key = ? WHERE id = ?', [key, row.id])
      .catch(() => {});
  }
...
async function refreshPatientIdentityKey(pool, patientId) {
  ...
  const active =
    ...
    st === 1 ||
    st === true ||
    String(st).toLowerCase() === '1' ||
    String(st).toLowerCase() === 'true';
  if (!row || !active) {
    await pool.query('UPDATE tbl_patient SET patient_identity_key = NULL WHERE id = ?', [pid]).catch(() => {});
    return;
  }
  const key = patientIdentityCompositeKey(row);
  await pool
    .query('UPDATE tbl_patient SET patient_identity_key = ? WHERE id = ?', [key || null, pid])
    .catch(() => {});
}

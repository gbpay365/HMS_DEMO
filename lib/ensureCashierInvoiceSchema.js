...
async function ensureCashierInvoiceSchema(pool) {
  ...
  await addColumn(pool, table, 'billing_company_id', 'INT NULL');
  await addColumn(pool, table, 'cash_tendered', 'DECIMAL(12,2) NULL');
  await addColumn(pool, table, 'change_amount', 'DECIMAL(12,2) NULL');
  ...
  await ensureBillingCompanySchema(pool).catch(() => {});

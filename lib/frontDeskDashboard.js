...
async function fetchFrontDeskDashboard(pool, opts = {}) {
  ...
  const quickActions = [
    ...
    { code: 'payment', label: 'Validate payment code', url: '/payment-validity', icon: 'fa-check-circle', color: '#10b981', perm: 'front_desk.payment_code.validate|payment.validity.read' },
  ...
  return {
    ...
    panels: {
      ...
      paymentCodes: (paymentRows || []).map((r) => ({
        ...
        patient: `${r.first_name || ''} ${r.last_name || ''}`.trim(),
        department: r.department,
        validateUrl: `/payment-validity?code=${encodeURIComponent(r.payment_code || '')}`,
      })),
    },
    aclModel: model,
  };

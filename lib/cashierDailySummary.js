...
const { getStandardPaymentMethodKeys } = require('./cashierPaymentMethods');

/** Always listed in payment-method filter (matches cashier desk). */
const STANDARD_PAYMENT_METHOD_KEYS = getStandardPaymentMethodKeys();
...
function paymentMethodKey(method) {
  ...
  if (k === 'mobile money' || k === 'mobile_money' || k === 'mobile') return 'momo';
  if (k === 'orange money' || k === 'orange') return 'om';
...
function paymentMethodLabel(method) {
  const k = paymentMethodKey(method);
  const LABELS = {
    cash: 'Cash',
    momo: 'MOMO',
    om: 'OM',
  ...
  const raw = normalizePaymentMethod(String(method || '').trim());
  if (raw) {
    const rk = raw.toLowerCase();
    if (rk === 'orange' || rk === 'orange money') return 'OM';
    return raw;
  }
...
function buildFilterOptions(allRows) {
  const methodKeys = new Set(STANDARD_PAYMENT_METHOD_KEYS);
  for (const row of allRows) {
    if (row.payment_method_key) methodKeys.add(row.payment_method_key);
  }
  const ordered = [
    ...STANDARD_PAYMENT_METHOD_KEYS,
    ...[...methodKeys].filter((k) => !STANDARD_PAYMENT_METHOD_KEYS.includes(k)).sort(),
  ];
  return {
    categories: CASHIER_REPORT_CATEGORIES,
    methods: ordered.map((key) => ({ key, label: paymentMethodLabel(key) })),
  };
}

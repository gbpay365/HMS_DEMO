...
async function ensureHeaderColumns(pool) {
  ...
  await ensureJournalLineDecimalColumns(pool);
}

/** Railway PG migrated journal lines with BIGINT debit/credit — TVA splits need DECIMAL. */
async function ensureJournalLineDecimalColumns(pool) {
  if (pool?.driver !== 'postgres') return;
  if (!(await tableExists(pool, 'tbl_fin_journal_line'))) return;
  try {
    ...
    for (const col of cols || []) {
      if (String(col.data_type || '').toLowerCase() !== 'bigint') continue;
      const name = col.column_name;
      await pool.query(
        `ALTER TABLE tbl_fin_journal_line
           ALTER COLUMN ${name} TYPE DECIMAL(14,2)
           USING (${name}::numeric)`
      );
    }
  } catch (e) {
    console.warn('ensureJournalLineDecimalColumns:', e.message);
  }
}

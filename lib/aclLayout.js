...
const { isPrivilegedAccountRole } = require('./hmsStaffAccountGuard');
...
function staffHomeUrl(role, opts) {
  ...
  return code ? resolveCashierLandingUrl(portalUrl(code)) : '/portal/front-desk';
}
...
/** Preferred landing after login: role home portal (never dashboard). */
...
function isPortalCacheReady() {
  return _rolePortals.size > 0;
}

/** Load role↔portal cache if boot migrations have not finished yet (login race). */
async function ensurePortalCacheReady() {
  if (_rolePortals.size > 0) return true;
...
/** Admin / Super Admin use wildcard perms — nav grants must not restrict their menus. */
function roleUsesNavGrantFilter(role) {
  if (isPrivilegedAccountRole(role)) return false;
  return navAccessRuntime.roleUsesNavGrants(role);
}
...
function navItemVisible(item, userPerms, roleOpt, opts) {
  ...
  const role = String(roleOpt != null ? roleOpt : '');
  const code = String(item.code || '');
  const navGrantMode = roleUsesNavGrantFilter(role);

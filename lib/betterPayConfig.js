...
const DEFAULT_PAY_BASE_URL = 'https://betterpayme.com/pay';

/** Rewrite legacy pay.betterpay.online URLs saved in env/DB/config. */
function normalizePayBaseUrl(url) {
  const u = String(url || '').trim();
  if (!u) return u;
  return u.replace(/^https?:\/\/pay\.betterpay\.online/i, 'https://betterpayme.com');
}
...
function readFileConfig() {
  try {
    ...
    return {
      ...
      payBaseUrl: normalizePayBaseUrl(raw.pay_base_url || raw.payBaseUrl || ''),
...
async function resolve(pool, force = false) {
  ...
  let baseUrl = normalizePayBaseUrl(process.env.BETTERPAY_PAY_BASE_URL || '');
  ...
  if (pool) {
    try {
      ...
      if (dbBase) baseUrl = normalizePayBaseUrl(dbBase);
  ...
  cache = {
    partner,
    baseUrl: baseUrl || DEFAULT_PAY_BASE_URL,
    webhookSecret,
    statusUrl,
    apiKey,
    loadedAt: now,
...
async function saveSettings(pool, settings = {}) {
  ...
  const baseUrl = normalizePayBaseUrl(settings.pay_base_url || settings.payBaseUrl || '');

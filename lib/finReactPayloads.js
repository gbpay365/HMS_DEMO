...
function treasuryPayload(d) {
  const treasuryLabel = hmsCountry.isNigeria ? 'Cash & bank (23xxxx)' : 'Class 5 balance';
  const treasuryMoveLabel = hmsCountry.isNigeria ? 'Cash & bank movement' : 'Class 5 movement';
  return finPageData('treasury', 'treasury', {
    title: 'Treasury & bank',
    subtitle: d.asof ? `As at ${d.asof}` : '',
    stats: [
      { label: treasuryLabel, value: d.class5Balance, format: 'money' },
      { label: treasuryMoveLabel, value: d.class5Movement, format: 'money' },
...
function cashFlowPayload(d) {
  return finPageData('cash-flow', 'cashflow', {
    ...
    stats: [
      ...
      { label: hmsCountry.isNigeria ? 'Cash & bank movement' : 'Class 5 movement', value: d.m5, format: 'money' },

...
/** Keep PostgreSQL BIGSERIAL in sync after MySQL imports so new ledger rows get unique ids. */
async function syncCashierTxnIdSequence(pool) {
  if (!pool || pool.driver !== 'postgres') return;
  await pool
    .query(`CREATE SEQUENCE IF NOT EXISTS tbl_cashier_txn_id_seq`)
    .catch(() => {});
  await pool
    .query(
      `SELECT setval(
         COALESCE(
           pg_get_serial_sequence('tbl_cashier_txn', 'id'),
           'tbl_cashier_txn_id_seq'
         ),
         GREATEST(COALESCE((SELECT MAX(id) FROM tbl_cashier_txn), 0), 1),
         (SELECT COUNT(*) > 0 FROM tbl_cashier_txn)
       )`
    )
    .catch(() => {});
}

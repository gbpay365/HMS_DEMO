  ...
  const [rows] = await pool.query(
    ...
    WHERE COALESCE(p.status, 1) = 1
      AND (
        ...
        OR p.phone LIKE ? OR CAST(p.id AS CHAR) LIKE ?
        OR LOWER(COALESCE(p.patient_code, '')) LIKE ?
        OR LOWER(CONCAT('#PT', LPAD(CAST(p.id AS CHAR), 4, '0'))) LIKE ?
        OR LOWER(COALESCE(w.qr_token, '')) LIKE ?
      )
    ORDER BY p.last_name, p.first_name
    LIMIT ?`,
    [...joinParams, like, like, like, like, idLike, idLike, like, like, like, Math.min(250, Math.max(1, limit))]
  );
  return rows || [];
}

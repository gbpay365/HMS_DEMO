...
async function fetchCashierBreakdown(pool, bounds, opts = {}) {
  ...
  if (ids.length) {
    ...
    cashierMap = await loadCashierMapByEmployeeIds(pool, ids);
    const missingIds = ids.filter((id) => !cashierMap[id]);
    if (missingIds.length) {
      const [emps] = await pool
        .query(
          `SELECT id, first_name, last_name, username FROM tbl_employee WHERE id IN (${missingIds.map(() => '?').join(',')})`,
          missingIds
        )
        .catch(() => [[]]);
      for (const e of emps || []) {
  ...
  return rows.map((r) => ({
    user_id: r.user_id,
    cashier_code: cashierMap[r.user_id]?.cashier_code || null,
    cashier_identity: cashierMap[r.user_id]?.cashier_identity || null,
    cashier_name: cashierMap[r.user_id]?.display || names[r.user_id] || (r.user_id ? `User #${r.user_id}` : '—'),
...
async function buildCashierEodReport(pool, opts = {}) {
  ...
  let ledgerClosing = [];
  try {
    const { fetchLedgerClosingBalances } = require('./lib/cashierLedgerReport');
    ledgerClosing = await fetchLedgerClosingBalances(pool, { facilityId: fid, date: bounds.start });
  } catch (_) {
    ledgerClosing = [];
  }
  ...
  await pool.query(
    pool.driver === 'postgres'
      ? `INSERT INTO tbl_cashier_eod_reconciliation
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
         ON CONFLICT (facility_id, business_date, cashier_user_id) DO UPDATE SET
           status = EXCLUDED.status,
           opening_float = EXCLUDED.opening_float,
           system_totals_json = EXCLUDED.system_totals_json,
           declared_totals_json = EXCLUDED.declared_totals_json,

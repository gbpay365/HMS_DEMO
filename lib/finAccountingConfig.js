...
const { vatRateDefault, vatCollectedAccount, vatDeductibleAccount } = require('./finGlAccountMaps');
...
/** @deprecated Use vatRateDefault() — kept for callers expecting live profile rate */
function getDefaultTvaRate() {
  return vatRateDefault();
}

/** When true, receipt/expense auto-post splits net + VAT lines (rate from active country profile). */
...
const STRICT_JOURNAL_RULES = {
  ...
  minLineAmount: 0.01,
  /** source_type values that skip normal-balance and duplicate-account rules. */
  exemptSourceTypes: ['reversal', 'reimputation', 'cashier_payout', 'cashier_disbursement'],
...
module.exports = {
  ...
  JOURNAL_CODES,
  SOURCE_JOURNAL_MAP,
  get DEFAULT_TVA_RATE() {
    return vatRateDefault();
  },
  get TVA_COLLECTED_ACCOUNT() {
    return vatCollectedAccount();
  },
  get TVA_DEDUCTIBLE_ACCOUNT() {
    return vatDeductibleAccount();
  },
  getDefaultTvaRate,
  APPLY_TVA_ON_SYNC,

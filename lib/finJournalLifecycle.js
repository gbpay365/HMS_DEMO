...
const { resolveJournalEntryLines } = require('./finCoaSubAccount');
...
async function postDraftJournal(pool, facilityId, journalId, userId, opts = {}) {
  const fid = Math.max(1, parseInt(facilityId, 10) || 1);
  const uid = Math.max(0, parseInt(userId, 10) || 0);
  const header = await loadJournalHeader(pool, fid, journalId);
  ...
  const draftLines = await loadJournalLines(pool, journalId);
  const linePayload = draftLines.map((ln) => ({
    code: ln.account_code,
    label: ln.account_label,
    debit: ln.debit,
    credit: ln.credit,
    line_memo: ln.line_memo,
    tier_id: ln.tier_id,
    tva_rate: ln.tva_rate,
    tva_amount: ln.tva_amount,
  }));

  const resolved = await resolveJournalEntryLines(pool, linePayload, {
    facilityId: fid,
  ...
  if (!resolved.ok) {
    return {
      ok: false,
      needsSubAccounts: Boolean(resolved.needsSubAccounts),
      missing: resolved.missing || [],
      error: resolved.error || "Can't Post on Main Account , Please Create Sub Account and then Post the Transaction",
    };
  }
  ...
  if (!validated.ok) {
    return { ok: false, error: validated.error || 'Draft failed validation.' };
  }
  ...
  if (String(header.source_type) !== 'account_core') {
    scheduleJournalSyncToAccountCore(pool, journalId, fid);
  }

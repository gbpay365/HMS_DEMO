...
function validateSyscohadaAccountCode(code) {
  ...
  if (!Number.isFinite(cls) || cls < 1 || cls > 9) {
    const chartLabel = hmsCountry.isNigeria ? 'IFRS' : 'OHADA';
    return { ok: false, error: `Invalid ${chartLabel} class for account ${c} (first digit must be 1–9).` };
  }
  if (c.length < 2 || c.length > 20) {
    const fmtLabel = hmsCountry.isNigeria ? 'Nigeria IFRS' : 'SYSCOHADA';
    return { ok: false, error: `Account code ${c} must be 2–20 digits (${fmtLabel}).` };
  }
...
async function validateJournalAccounts(pool, lines, facilityId) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!acc) {
      return {
        ok: false,
        error: `Unknown or inactive account: ${code} (chart of accounts${hmsCountry.isNigeria ? ' / Nigeria IFRS' : ' / SYSCOHADA'}).`,
    ...
    if (parseInt(acc.is_posting, 10) !== 1) {
      return {
        ok: false,
        error: `Account ${code} is a mother/header account — create a postable sub-account (e.g. ${code.slice(0, -1)}1) before posting.`,
        accountsByCode: byCode,
      };
    }
    const [[childRow]] = await pool
      .query(
        `SELECT id FROM tbl_fin_account
          WHERE active = 1 AND is_posting = 1 AND parent_id = ?
          LIMIT 1`,
        [acc.id]
      )
      .catch(() => [[null]]);
    if (childRow?.id) {
      return {
        ok: false,
        error: `Account ${code} is a mother account with sub-accounts — post to a detail sub-account, not the parent.`,
        accountsByCode: byCode,
      };
    }
    if (!ln.label) {

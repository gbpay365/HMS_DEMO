...
/** True when a DATETIME/TIMESTAMP column is NULL or a legacy zero date — PostgreSQL compatible. */
function isZeroOrNullDateSql(col) {
  return `(${col} IS NULL OR EXTRACT(YEAR FROM ${col}) = 0)`;
}

/** Normalize legacy zero dates to NULL in SELECT/ORDER — PostgreSQL compatible (CASE WHEN instead of MySQL IF). */
function safeDateTimeSql(col) {
  return `CASE WHEN ${isZeroOrNullDateSql(col)} THEN NULL ELSE ${col} END`;
}

const NOT_DISCHARGED = isZeroOrNullDateSql('a.discharged_at');
const IS_DISCHARGED = `(a.discharged_at IS NOT NULL AND EXTRACT(YEAR FROM a.discharged_at) <> 0)`;
...
async function getInvoiceForecast(pool, admissionId) {
  ...
  const [[adm]] = await pool.query(
    `SELECT a.id, a.deposit_amount, a.running_bill, ${safeAdm} AS admitted_at,
            (CURRENT_DATE - (${safeAdm})::date) AS los_days
     FROM tbl_admission a WHERE a.id=$1 LIMIT 1`,
    [admissionId]
  );
  if (!adm) return null;

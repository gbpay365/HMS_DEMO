...
async function loadJournalLines(pool, journalId) {
  ...
  return (rows || [])
    .map((ln) => ({
      account_code: String(ln.account_code || '').trim().slice(0, 6),
...
async function syncOneJournalToAccountCore(pool, header, opts = {}) {
  ...
  if (!core.enabled) return { ok: false, skipped: true, reason: 'Core sync disabled (env or Super Admin integrations)' };
  const url = core.url;
  const key = core.key;
  if (!url || !key) return { ok: false, skipped: true, reason: 'missing Core URL or API key (check Super Admin → Integrations)' };
...
async function syncAllJournalsToAccountCore(pool, opts = {}) {
  ...
  const core = await resolveCoreAccountConfig(pool, facilityId);
  if (!core.enabled) {
    return { ok: false, error: 'Core sync disabled. Enable in Railway env (CORE_ACCOUNT_SYNC_ENABLED=1) or Super Admin → Integrations.' };
  }
  if (!core.url || !core.key) {
    return { ok: false, error: 'Missing Core URL or API key. Set CORE_ACCOUNT_URL / CORE_ACCOUNT_WEBHOOK_KEY or Super Admin → Integrations.' };
  }
...
/** Count journals by sync status for diagnostics. */

...
async function syncOneJournalToAccountCore(pool, header, opts = {}) {
  ...
  let payMethod = '';
  if (String(header.source_type || '') === 'billing_receipt') {
    const [[doc]] = await pool
      .query('SELECT payment_method FROM tbl_billing_document WHERE id = ? LIMIT 1', [header.source_id])
      .catch(() => [[null]]);
    payMethod = String(doc?.payment_method || '').trim();
  }

  const entryDate = toIsoDatePart(header.entry_date) || new Date().toISOString().slice(0, 10);
  const payPrefix = payMethod ? `${payMethod} · ` : '';
  ...
  const body = {
    ...
    description: `${payPrefix}${String(header.narration || header.reference || `HMS journal ${jid}`)}`.slice(0, 512),

...
async function setPharmacyProductPrice(pool, opts) {
  const item = await loadPharmacyInventoryItem(pool, opts.itemId);
  if (!item) return { ok: false, error: 'Pharmacy product not found.' };
  const catalogId = parseInt(item.catalog_id || item.service_catalog_id, 10) || 0;
  if (catalogId < 1) return { ok: false, error: 'Product is not linked to the service catalog.' };
  const price = Math.max(0, parseFloat(opts.price) || 0);
  const [upd] = await pool.query(
    `UPDATE tbl_service_catalog SET price = ?
     WHERE id = ? AND LOWER(TRIM(category)) = 'pharmacy' AND status = 1
     LIMIT 1`,
    [price, catalogId]
  );
  if (!upd || upd.affectedRows < 1) {
    return { ok: false, error: 'Could not update catalog price for this product.' };
  }
  await pool.query('UPDATE tbl_inventory_item SET unit_price = ? WHERE id = ? LIMIT 1', [price, item.id]);

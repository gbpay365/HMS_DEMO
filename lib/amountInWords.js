...
/**
 * @param {number|string} amount — total paid (FCFA / XAF); may include centimes
...
function amountPaidWords(amount) {
  ...
  let en = _cap(integerToEnglishWords(whole)) + ' francs CFA';
  if (cents > 0) {
    en += ' and ' + integerToEnglishWords(cents) + ' centime' + (cents === 1 ? '' : 's');
  }
  en += ' only';

  // French line: same numerals are standard on bilingual forms; add clear French legal phrasing.
  const fr = (
    _cap(integerToFrenchWords(whole)) + ' francs CFA' +
    (cents > 0 ? (' et ' + integerToFrenchWords(cents) + ' centime' + (cents === 1 ? '' : 's')) : '') +
    ' seulement'
  );
  ...
  return {
    en,
    fr,
    numeric: rounded.toLocaleString('fr-FR', { minimumFractionDigits: cents ? 2 : 0, maximumFractionDigits: 2 })
  };
}

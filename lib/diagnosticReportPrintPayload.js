...
function infoSchemaColumnName(row) {
  return row.COLUMN_NAME || row.column_name;
}

function isMissingColumnError(err) {
  const msg = String(err?.message || '');
  return /Unknown column/i.test(msg) || /column .+ does not exist/i.test(msg);
}

async function loadRadiologyResultColumnSet(pool) {
  const schemaClause =
    pool && pool.driver === 'postgres'
      ? "table_schema = 'public'"
      : 'TABLE_SCHEMA = DATABASE()';
  try {
    const [rows] = await pool.query(
      `SELECT column_name AS COLUMN_NAME FROM information_schema.columns
       WHERE ${schemaClause} AND table_name = 'tbl_radiology_result'`
    );
    return new Set((rows || []).map(infoSchemaColumnName).filter(Boolean));
  } catch {
    return new Set(['exam_name', 'test_name', 'structured_result', 'status', 'revision_pending']);
  }
}
...
async function loadRadPrintPayload(pool, id) {
  const cols = await loadRadiologyResultColumnSet(pool);

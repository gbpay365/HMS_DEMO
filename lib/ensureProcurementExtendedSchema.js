...
async function ensureProcurementExtendedSchema(pool) {
  if (pool.driver === 'postgres') {
    if (!(await tableExists(pool, 'tbl_procurement_purchase_request'))) {
      await pool.query(`
        CREATE TABLE IF NOT EXISTS tbl_procurement_purchase_request (
          ...
          converted_po_id INTEGER DEFAULT NULL,
          source_department VARCHAR(32) DEFAULT NULL,
          source_requisition_id INTEGER DEFAULT NULL
        )
      `);
      await pool
        .query(
          `CREATE UNIQUE INDEX IF NOT EXISTS uq_pr_num ON tbl_procurement_purchase_request (facility_id, pr_number)`
        )
        .catch(() => {});
      await pool
        .query(`CREATE INDEX IF NOT EXISTS ix_pr_fac ON tbl_procurement_purchase_request (facility_id)`)
        .catch(() => {});
      await pool
        .query(`CREATE INDEX IF NOT EXISTS ix_pr_status ON tbl_procurement_purchase_request (status)`)
        .catch(() => {});
    } else {
      await addCol(
        pool,
        'tbl_procurement_purchase_request',
        'source_department',
        'source_department VARCHAR(32) DEFAULT NULL'
      );
      await addCol(
    ...
    if (!(await tableExists(pool, 'tbl_procurement_purchase_request_line'))) {
      ...
      await pool
        .query(
          `CREATE INDEX IF NOT EXISTS ix_prl_pr ON tbl_procurement_purchase_request_line (purchase_request_id)`
        )
        .catch(() => {});
    }
    return;
  ...
  await addCol(pool, 'tbl_procurement_purchase_request', 'source_department', 'source_department VARCHAR(32) DEFAULT NULL');
  await addCol(pool, 'tbl_procurement_purchase_request', 'source_requisition_id', 'source_requisition_id INT DEFAULT NULL');

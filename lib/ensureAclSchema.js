...
module.exports = async function ensureAclSchema(pool) {
  ...
  const newPerms = [
    ...
    ['maternity.register.new_patient', 'Maternity: register new mother from ANC booking', 'maternity'],
  ...
  const elements = [
    ...
    ['mat.btn.register_new_anc', 'global', 'button', 'Register New ANC', '/patients?from=maternity&action=new', 'fa-user-plus', '#9d174d', 'maternity.register.new_patient', 13, null],
    ...
    ['fd.tile.visit',     'front_desk', 'tile', 'Create New Visit',  '/opd-queue?action=new',          'fa-plus-circle',     '#1d4ed8', 'front_desk.visit.create|opd.write|scheduling.write|opd.read|scheduling.read', 15, null],
    ...
    ['fd.tile.payment',   'front_desk', 'tile', 'Validate Payment Code', '/front-desk/validate-payment-code', 'fa-check-circle', '#10b981', 'front_desk.payment_code.validate|payment.validity.read|opd.read|patient.read', 35, null],
  ...
  /** Doctor roles: hide mis-cloned doctor-portal executive tiles + hospital-wide dashboard. */
  const doctorHubHide = [
  ...
  const actionMenuElements = [
    ...
    ['am.opd_queue.new_visit', 'action_menus', 'action_menu', 'Register new visit', null, 'fa-user-plus', '#047857', 'front_desk.visit.create|opd.write|scheduling.write', 5, 'opd_queue'],
    ...
    ['ward.hero.ipd_settle', 'global', 'button', 'IPD settle (ward board)', '/cashier/ipd-settle', 'fa-credit-card', '#10b981', 'cashier.write|billing.write', 10, null],
    ['ward.hero.manage_beds', 'global', 'button', 'Manage beds (ward board)', null, 'fa-bed', '#0ea5e9', 'adt.write|facility.admin', 11, null],
  ...
  const defaultRolePerms = {
    '2':  ['patient.read','patient.write','patient.delete','patient_portal.manage','chart.read','chart.write','profile.self.write','hr.self.read',
           ...
           'maternity.read','maternity.write','maternity.register.new_patient',
    ...
    '7':  ['nursing.read','nursing.write','nurse_duty.read','nurse_duty.write','patient.read','patient.write','patient_portal.manage','chart.read','chart.write','adt.read',
           'clinical.read','opd.read','ipd_medication.read','ipd_medication.write','maternity.read','maternity.write','maternity.register.new_patient','vaccination.read','vaccination.write','profile.self.write','hr.self.read'],
  ...
  await runAclBootstrapOnce(pool, 'bootstrap.doctor_hide_hms_dashboard_v1', async () => {
    const doctorRoles = new Set(['2', '100']);
    try {
      const [docRoles] = await pool.query(
        `SELECT DISTINCT CAST(role AS CHAR) AS role FROM tbl_role
          WHERE LOWER(title) LIKE '%doctor%' OR LOWER(title) LIKE '%physician%'`
    ...
    for (const role of doctorRoles) {
      await pool.query(
        `INSERT IGNORE INTO tbl_acl_role_ui_hidden (role, element_code) VALUES (?, 'sb.dashboard')`,
        [role]
      ).catch(() => {});
      await pool.query(
        `DELETE rp FROM tbl_acl_role_permission rp
         INNER JOIN tbl_acl_permission p ON p.id = rp.permission_id
         WHERE rp.role = ? AND p.code = 'dashboard.read'`,
        [role]
      ).catch(() => {});
    }
  });

  /** Lab technician home portal: hide hospital-wide /dashboard (lab portal is the landing). */
  await runAclBootstrapOnce(pool, 'bootstrap.labtech_hide_hms_dashboard_v1', async () => {
    const labtechRoles = new Set(['4', '102']);
    try {
      ...
      for (const row of rows || []) {
        if (row && row.role != null) labtechRoles.add(String(row.role));
      }
      const [homeRoles] = await pool.query(
        `SELECT DISTINCT CAST(role AS CHAR) AS role FROM tbl_acl_role_portal
          WHERE portal_code IN ('labtech', 'laboratory', 'lab_tech') AND is_home = 1`
      );
      for (const r of homeRoles || []) {
        if (r.role != null) labtechRoles.add(String(r.role));
      }
    } catch (_) { /* optional */ }
    for (const role of labtechRoles) {
      await pool.query(
        `INSERT IGNORE INTO tbl_acl_role_ui_hidden (role, element_code) VALUES (?, 'sb.dashboard')`,
        [role]
      ).catch(() => {});
  ...
  /** Radiologist home portal: hide hospital-wide /dashboard (radiology portal is the landing). */
  await runAclBootstrapOnce(pool, 'bootstrap.radiologist_hide_hms_dashboard_v1', async () => {
    const radiologistRoles = new Set(['6', '104']);
    try {
      const [rows] = await pool.query(
        `SELECT CAST(role AS CHAR) AS role FROM tbl_role
      ...
      for (const row of rows || []) {
        if (row && row.role != null) radiologistRoles.add(String(row.role));
      }
      const [homeRoles] = await pool.query(
        `SELECT DISTINCT CAST(role AS CHAR) AS role FROM tbl_acl_role_portal
          WHERE portal_code IN ('radiology', 'radiologist') AND is_home = 1`
      );
      for (const r of homeRoles || []) {
        if (r.role != null) radiologistRoles.add(String(r.role));
      }
    } catch (_) { /* optional */ }
  ...
  await runAclBootstrapOnce(pool, 'bootstrap.opd_new_visit_acl_v1', async () => {
    for (const role of ['3', '101']) {
      await pool.query(
        `INSERT IGNORE INTO tbl_acl_role_permission (role, permission_id)
         SELECT ?, id FROM tbl_acl_permission WHERE code = 'front_desk.visit.create' LIMIT 1`,
        [role]
      ).catch(() => {});
    }
    await pool.query(
      `INSERT IGNORE INTO tbl_acl_role_permission (role, permission_id)
       SELECT rp.role, p.id
         FROM tbl_acl_role_portal rp
         JOIN tbl_acl_permission p ON p.code = 'front_desk.visit.create'
        WHERE rp.portal_code = 'front_desk'`
    ).catch(() => {});
  });

  await runAclBootstrapOnce(pool, 'bootstrap.opd_new_visit_acl_v2', async () => {
    await pool.query(
      `UPDATE tbl_acl_ui_element SET required_perm = 'front_desk.visit.create|opd.write|scheduling.write'
       WHERE code = 'am.opd_queue.new_visit'`
    ).catch(() => {});
    await pool.query(
      `INSERT IGNORE INTO tbl_acl_role_permission (role, permission_id)
       SELECT DISTINCT rp.role, p.id
         FROM tbl_acl_role_permission rp
         JOIN tbl_acl_permission src ON src.id = rp.permission_id
         JOIN tbl_acl_permission p ON p.code = 'front_desk.visit.create'
        WHERE src.code IN ('scheduling.write', 'opd.write', 'clinical.write')`
    ).catch(() => {});
  });

  await runAclBootstrapOnce(pool, 'bootstrap.mat_register_new_patient_v1', async () => {
    const codes = ['maternity.register.new_patient', 'patient.write'];
    for (const role of ['2', '7', '103']) {
      for (const code of codes) {
        await pool.query(
          `INSERT IGNORE INTO tbl_acl_role_permission (role, permission_id)
           SELECT ?, id FROM tbl_acl_permission WHERE code = ? LIMIT 1`,

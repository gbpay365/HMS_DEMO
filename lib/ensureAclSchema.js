...
module.exports = async function ensureAclSchema(pool) {
  ...
  const elements = [
    ...
    ['fd.tile.visit',     'front_desk', 'tile', 'Create New Visit',  '/opd-queue?action=new',          'fa-plus-circle',     '#1d4ed8', 'front_desk.visit.create', 15, null],
  ...
  const actionMenuElements = [
    ...
    ['am.opd_queue.new_visit', 'action_menus', 'action_menu', 'Register new visit', null, 'fa-user-plus', '#047857', 'front_desk.visit.create', 5, 'opd_queue'],
  ...
  await require('./repairOpdVisitBillingAcl').repairOpdVisitBillingAcl(pool);

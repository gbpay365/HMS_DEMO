...
function isPgPool(pool) {
  return String(pool?.driver || '').toLowerCase() === 'postgres';
}
...
async function ensureSchema(pool) {
  ...
  if (isPgPool(pool)) {
    await pool
      .query('CREATE UNIQUE INDEX IF NOT EXISTS uq_betterpay_ref ON tbl_betterpay_payment (ref)')
      .catch(() => {});
  }
}
...
async function upsertPending(pool, { ref, patientId, amount, meta }) {
  ...
  const usePg = isPgPool(pool);
  // #region agent log
  fetch('http://127.0.0.1:7824/ingest/7799ec2f-1013-4dae-a65a-dcfd2e3f62ad',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'968473'},body:JSON.stringify({sessionId:'968473',location:'betterPayPayment.js:upsertPending',message:'upsert pending start',data:{ref:refStr,patientId:patientId||null,amount:amt,usePg},timestamp:Date.now(),hypothesisId:'H1',runId:'betterpay-fix'})}).catch(()=>{});
  // #endregion
  try {
    if (usePg) {
      await pool.query(
        `INSERT INTO tbl_betterpay_payment (ref, patient_id, amount, status, meta_json, waiting_started_at, created_at, updated_at)
         ...
         ON CONFLICT (ref) DO UPDATE SET
           ...
           status = CASE WHEN tbl_betterpay_payment.status = 'paid' THEN 'paid' ELSE 'pending' END,
           meta_json = COALESCE(EXCLUDED.meta_json, tbl_betterpay_payment.meta_json),
           waiting_started_at = CASE
             WHEN tbl_betterpay_payment.status = 'paid' THEN tbl_betterpay_payment.waiting_started_at
             ELSE NOW()
           END,
           updated_at = NOW()`,
    ...
    // #region agent log
    fetch('http://127.0.0.1:7824/ingest/7799ec2f-1013-4dae-a65a-dcfd2e3f62ad',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'968473'},body:JSON.stringify({sessionId:'968473',location:'betterPayPayment.js:upsertPending',message:'upsert pending ok',data:{ref:refStr,usePg},timestamp:Date.now(),hypothesisId:'H1',runId:'betterpay-fix'})}).catch(()=>{});
    // #endregion
  } catch (e) {
    // #region agent log
    fetch('http://127.0.0.1:7824/ingest/7799ec2f-1013-4dae-a65a-dcfd2e3f62ad',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'968473'},body:JSON.stringify({sessionId:'968473',location:'betterPayPayment.js:upsertPending',message:'upsert pending error',data:{ref:refStr,usePg,error:String(e.message||e)},timestamp:Date.now(),hypothesisId:'H1',runId:'betterpay-fix'})}).catch(()=>{});
    // #endregion
    throw e;
  }
  return refStr;
...
async function purgeExpired(pool) {
  await ensureSchema(pool);
  const expiredSql = isPgPool(pool)
  ...
  const [rows] = await pool.query(expiredSql, [FAILED_RETENTION_HOURS]);
  for (const r of rows || []) {

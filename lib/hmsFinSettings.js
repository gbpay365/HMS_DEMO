...
async function loadTaxSettings(pool) {
  return {
    ...
    company_city: await getFinSetting(pool, 'company.city', 'Douala'),
    tva_rate_standard: await getFinSetting(pool, 'tax.tva_rate_standard', '19.25'),
    cnps_employer_pct: await getFinSetting(pool, 'tax.cnps_employer_pct', '8.4'),
    currency_code: await getFinSetting(pool, 'company.currency', 'XAF'),
    fiscal_regime: await getFinSetting(pool, 'company.fiscal_regime', 'SYSCOHADA / OHADA'),
    chart_template: await getFinSetting(pool, 'accounting.chart', 'SYSCOHADA'),
  };
}

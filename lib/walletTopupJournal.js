 ...
 * Replicates to Account Core via journalPostExtended → maybeReplicateJournalToCoreAccount.
 */
async function postWalletTopupJournal(pool, opts) {
  ...
  // #region agent log
  fetch('http://127.0.0.1:7824/ingest/7799ec2f-1013-4dae-a65a-dcfd2e3f62ad',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'968473'},body:JSON.stringify({sessionId:'968473',location:'walletTopupJournal.js:postWalletTopupJournal',message:'wallet topup journal posted',data:{txnId,journalId:r.journalId||0,ok:!!(r.ok||r.duplicate),duplicate:!!r.duplicate,amount},timestamp:Date.now(),hypothesisId:'WT1',runId:'wallet-topup'})}).catch(()=>{});
  // #endregion

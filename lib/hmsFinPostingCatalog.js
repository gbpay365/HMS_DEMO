...
function normalizePostingAccount(row, codeCol, labelCol) {
  ...
  const ohadaClass = row.ohada_class != null
    ? parseInt(row.ohada_class, 10) || 0
    : (/^\d/.test(code) ? parseInt(code[0], 10) : 0);
  return {
    id: row.id,
    code,
    account_code: code,
...
/** SYSCOHADA normal balance side (matches finJournalValidation / Account_Core). */
function normalSideForAccountType(accountType) {
...
async function loadPostingAccounts(pool) {
  ...
  const hasClass = await columnExists(pool, 'tbl_fin_account', 'ohada_class');
  const typeExpr = hasType ? 'account_type' : "'' AS account_type";
  const classExpr = hasClass ? 'ohada_class' : '0 AS ohada_class';
  ...
  try {
    const [rows] = await pool.query(
      `SELECT id, ${codeCol} AS code, ${labelExpr} AS label_disp, ${typeExpr}, ${classExpr}${hasSort ? ', sort_order' : ', 0 AS sort_order'}
...
module.exports = { loadPostingAccounts, searchPostingAccounts, normalizePostingAccount, normalSideForAccountType };

...
const { formatMoney: hmsFormatMoney, isNigeria } = require('./hmsMoneyFormat');
...
function formatXaf(n, withCurrency = true) {
 const v = Number(n) || 0;
 const loc = isNigeria ? 'en-NG' : 'fr-FR';
 const s = Math.round(v).toLocaleString(loc, { minimumFractionDigits: 0, maximumFractionDigits: 2 });
 return withCurrency ? hmsFormatMoney(v) : s;
}

...
function lineItemsFromTicketLines(lines) {
  return (lines || [])
    ...
    .map((l) => {
      const unitPrice = Number(l.list_unit_price || l.unit_price || 0) || 0;
      const quantity = Number(l.quantity || 1) || 1;
      const patientDue = Number(l.patient_due);
      const amount = Number.isFinite(patientDue) && patientDue >= 0
        ? patientDue
        : Number(l.amount || l.total || 0) || unitPrice * quantity;
      return {
        description:
          l.description ||
          l.name ||
        ...
        quantity,
        amount,
        patient_due: Number.isFinite(patientDue) ? patientDue : undefined,
        comments: String(l.comments || '').trim() || null,

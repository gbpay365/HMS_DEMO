...
async function syncCashierTxnToCoreAccount(pool, txnId) {
  ...
  const payLabel = String(row.payment_method || 'Cash').trim() || 'Cash';
  const baseNarration = String(row.narration || row.txn_type || 'receipt').trim();
  ...
  const body = isOutflow
    ? {
        ...
        narration: `${payLabel} · ${row.cashier_code} ${row.cashier_identity} · ${baseNarration}`.slice(0, 500),
    ...
    : {
        ...
        narration: `${payLabel} receipt · ${row.cashier_code} ${row.cashier_identity} · ${baseNarration}`.slice(0, 500),
        amount_ttc: parseFloat(row.amount) || 0,
        payment_method: row.payment_method || 'Cash',
        service_key: 'default',
        gl_debit_account: row.gl_debit_account || '',

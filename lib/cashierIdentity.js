...
function activeWhere(alias, pool) {
  return catalogActiveWhere(alias, pool);
}
...
async function deactivateCashierForEmployee(pool, employeeId) {
  ...
  const [r] = await pool
    .query(
      `UPDATE tbl_cashier SET status = 0, updated_at = NOW() WHERE employee_id = ? AND ${activeWhere('', pool)}`,
      [uid]
    )
...
async function listCashierEmployeeIds(pool, facilityId = 1) {
  ...
  let sql = `
    SELECT e.id
      FROM tbl_employee e
     WHERE ${activeWhere('e', pool)}
       AND CAST(e.role AS CHAR) IN (${placeholders})`;
  if (facilityId > 0) {
    sql += ' AND (e.facility_id IS NULL OR e.facility_id = ? OR e.facility_id = 0)';
  ...
  const [rows] = await pool
    .query(
      `SELECT cashier_code FROM tbl_cashier
       WHERE facility_id = ? AND ${activeWhere('', pool)}
       ORDER BY id ASC`,
      [facilityId]
    )
    .catch(() => [[]]);
...
async function syncCashierIdentities(pool, opts = {}) {
  ...
  const [existing] = await pool
    .query(
      `SELECT employee_id FROM tbl_cashier
       WHERE employee_id IN (${placeholders}) AND ${activeWhere('', pool)}`,
      employeeIds
    )
    .catch(() => [[]]);
  ...
  for (const employeeId of missing) {
    ...
    try {
      ...
      /* race or duplicate — skip */
    }
  }
  return { created, total: employeeIds.length };
}

async function assignCashierToEmployee(pool, employeeId, facilityId = 1, opts = {}) {
  const uid = parseInt(String(employeeId || ''), 10) || 0;
  if (uid < 1) return null;
  if (!opts.skipSchemaEnsure) {
    await ensureCashierIdentitySchema(pool);
  }
  ...
  const [[existing]] = await pool
    .query(
      `SELECT id, facility_id, cashier_code, cashier_identity, employee_id, status
         FROM tbl_cashier
        WHERE employee_id = ? AND ${activeWhere('', pool)}
        LIMIT 1`,
      [uid]
    )
    .catch(() => [[null]]);
  ...
  const [[emp]] = await pool
    .query(`SELECT id FROM tbl_employee WHERE id = ? AND ${activeWhere('', pool)} LIMIT 1`, [uid])
    .catch(() => [[null]]);
  if (!emp) return null;
...
async function resolveCashierForEmployee(pool, employeeId, opts = {}) {
  ...
  const roles = await fetchCashierRoles(pool);
  const [[emp]] = await pool
    .query(`SELECT id, role FROM tbl_employee WHERE id = ? AND ${activeWhere('', pool)} LIMIT 1`, [uid])
    .catch(() => [[null]]);
  if (!emp) return null;
  ...
  if (opts.forceAssign) {
    return assignCashierToEmployee(pool, uid, facilityId, opts);
  }
  return null;
}

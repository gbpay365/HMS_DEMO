    ...
    if (pendingRows.length && String(pendingRows[0].status) === 'pending') {
      ...
      await conn.query(
        `UPDATE tbl_payment_ticket
         ...
         WHERE id = ?`,
        [amtDue, payMethod, JSON.stringify(ctx.lines), userId, ctx.paymentNote || null, newTicketId]
      );
    } else if (!pendingRows.length) {
      const [insResult] = await conn.query(
        `INSERT INTO tbl_payment_ticket
         (facility_id, ticket_code, patient_id, total_amount, status, payment_method, lines_json, created_by, paid_at, created_at, notes)
         VALUES (?, ?, ?, ?, 'paid', ?, ?, ?, NOW(), NOW(), ?)`,
        [fid, ctx.ticket_code, ctx.pid, amtDue, payMethod, JSON.stringify(ctx.lines), userId, ctx.paymentNote || null]
      );

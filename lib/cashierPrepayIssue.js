...
const { parseCardMetaFromBody, formatCardMetaNote } = require('./cardPaymentMeta');

const PAYMENT_NOT_RECEIVED_MSG = 'Ticket cannot be issued, payment not received';
const WALLET_NO_ACCOUNT_MSG =
  'No active wallet for this patient. Create or top up the wallet at Wallet hub first.';
const WALLET_INSUFFICIENT_MSG =
  'Insufficient wallet balance. Top up the patient wallet and try again.';
...
function parsePrepayLineRequests(body) {
  ...
  if (normalized.length) {
    return normalized.map((row) => ({
      ...
      prepay_quantity: row.prepay_quantity ?? row.quantity,
      prepay_comments: row.prepay_comments ?? row.comments,
  ...
  if (catalogId) {
    return [{
      ...
      prepay_quantity: body.prepay_quantity ?? body.quantity,
      prepay_comments: body.prepay_comments ?? body.comments,
    }];
  }
  return [];
}
...
async function buildPrepayTicketLine(pool, lineReq, ctx) {
  let serviceType = ['consultation', 'laboratory', 'radiology', 'hospitalisation', 'maternity', 'surgery', 'pharmacy', 'scan'].includes(
  ...
  const qty = Math.max(1, parseInt(lineReq.prepay_quantity ?? lineReq.quantity, 10) || 1);
  const comments = String(lineReq.prepay_comments ?? lineReq.comments ?? '').trim().slice(0, 500);
  const serviceName = cat.name || 'Service';
  const dept = (cat.department_name || '').trim();
  const lineTotal = Math.round(basePrice * qty * 100) / 100;
  const insurerAmount = Math.round(lineTotal * ctx.coveragePct / 100);
  const patientDue = lineTotal - insurerAmount;
...
async function ensurePendingPaymentTicket(pool, ctx, session) {
  ...
  if (existing.length) {
    ...
    const { assignInvoiceNumberIfMissing } = require('./paymentTicketInvoiceNumber');
    await assignInvoiceNumberIfMissing(pool, existing[0].id, fid).catch(() => {});
  ...
  const { assignInvoiceNumberIfMissing } = require('./paymentTicketInvoiceNumber');
  await assignInvoiceNumberIfMissing(pool, ins.insertId, fid).catch(() => {});
...
async function resolvePrepayContext(pool, body) {
  ...
  const cardMeta = parseCardMetaFromBody(body);
  const paymentNote = cardMeta ? formatCardMetaNote(cardMeta) : '';
  const cashMeta = parseCashPaymentMeta(body, payMethod, patientDue);
  ...
  return {
    ...
    cardMeta,
    paymentNote,
    cashTendered: cashMeta.cashTendered,
    changeAmount: cashMeta.changeAmount,
  };
}

function parseCashPaymentMeta(body, payMethod, amountDue) {
  if (payMethod !== 'Cash') return { cashTendered: null, changeAmount: null };
  const tendered = parseFloat(body?.prepay_cash_tendered);
  const due = parseFloat(amountDue) || 0;
  if (!Number.isFinite(tendered) || tendered <= 0) return { cashTendered: null, changeAmount: null };
  const change = Math.round((tendered - due) * 100) / 100;
  return {
    cashTendered: Math.round(tendered * 100) / 100,
...
async function issuePrepayTicket(pool, ctx, session) {
  ...
  try {
    ...
    if (payMethod === 'Wallet') {
      const { findWalletForPatient } = require('./walletHub');
      const wallet = await findWalletForPatient(conn, ctx.pid, fid);
      // #region agent log
      // #endregion
      if (!wallet || String(wallet.status || '').toLowerCase() !== 'active') {
        await conn.rollback();
        return { ok: false, error: WALLET_NO_ACCOUNT_MSG, status: 402 };
      }
      ...
      if (walletBal < amtDue) {
        ...
        return {
          ok: false,
          error: `${WALLET_INSUFFICIENT_MSG} (balance: ${hmsFormatMoney(walletBal)}, required: ${hmsFormatMoney(amtDue)})`,
          status: 402,
        };
      }
      walletHold = wallet;
      ...
      const [[locked]] = await conn
        .query('SELECT id, balance FROM tbl_patient_wallet WHERE id = ? FOR UPDATE', [wallet.id])
        .catch(() => [[null]]);
      if (!locked) {
        await conn.rollback();
        return { ok: false, error: WALLET_NO_ACCOUNT_MSG, status: 402 };
      }
      const lockedBal = parseFloat(locked.balance) || 0;
      if (lockedBal < amtDue) {
        await conn.rollback();
        return {
          ok: false,
          error: `${WALLET_INSUFFICIENT_MSG} (balance: ${hmsFormatMoney(lockedBal)}, required: ${hmsFormatMoney(amtDue)})`,
          status: 402,
        };
      }
      walletHold = locked;
      walletNewBal = lockedBal - amtDue;
    }
    ...
    if (pendingRows.length && String(pendingRows[0].status) === 'pending') {
      ...
      await conn.query(
        `UPDATE tbl_payment_ticket
         SET total_amount = ?, status = 'paid', payment_method = ?, lines_json = ?, paid_at = NOW(), created_by = COALESCE(created_by, ?),
             notes = COALESCE(?, notes), cash_tendered = ?, change_amount = ?
         WHERE id = ?`,
        [
          amtDue,
          payMethod,
      ...
      const [insResult] = await conn.query(
        `INSERT INTO tbl_payment_ticket
         (facility_id, ticket_code, patient_id, total_amount, status, payment_method, lines_json, created_by, paid_at, created_at, notes, cash_tendered, change_amount)
         VALUES (?, ?, ?, ?, 'paid', ?, ?, ?, NOW(), NOW(), ?, ?, ?)`,
        [
          fid,
          ctx.ticket_code,
    ...
    const { resolveInvoiceNumberForTicket } = require('./paymentTicketInvoiceNumber');
    const receiptNo = await nextReceiptNumber(conn, fid);
    const invoiceNo = await resolveInvoiceNumberForTicket(conn, newTicketId, fid);
    const [docIns] = await conn.query(
      ...
      [fid, ctx.pid, receiptNo, invoiceNo, amtDue, payMethod, newTicketId, userId]
    );
    ...
    cashierTxnResult = await optionalInTransaction(conn, 'cashier_txn_prepay', async () => {
      const { recordReceiptInTransaction } = require('./cashierTxnWire');
      return recordReceiptInTransaction(conn, {
        facilityId: fid,
        userId,
        sourceModule: 'payment_ticket',
        sourcePk: newTicketId,
        amount: amtDue,
        paymentMethod: payMethod,
        billingDocumentId: billingDocId || null,
        patientId: ctx.pid,
        lines: ctx.lines,
        ticket: { ticket_code: ctx.ticket_code, ticket_category: ctx.lines[0]?.kind },
        reference: receiptNo,
        narration: `Prepay ${ctx.ticket_code}`,
      });
    });
    ...
    const [[verify]] = await pool.query(
      'SELECT id FROM tbl_payment_ticket WHERE TRIM(ticket_code) = ? LIMIT 1',
      [String(ctx.ticket_code || '').trim()]
    );
    if (!verify) {
      return { ok: false, error: 'Ticket could not be saved. Please try again.', status: 500 };
    }
    ...
    try {
      ...
      await runCashierPostCommit(pool, {
        ...
        createdBy: userId,
        docNumber: receiptNo,
        firstLineDescription: ctx.serviceName || ctx.ticket_code,
        sourceModule: 'payment_ticket',
      });
    } catch (pipeErr) {
      console.error('cashier journal pipeline (prepay):', pipeErr.message);
    }

...
async function issuePrepayTicket(pool, ctx, session) {
  ...
  try {
    ...
    if (payMethod === 'Wallet') {
      ...
      // #region agent log
      fetch('http://127.0.0.1:7824/ingest/7799ec2f-1013-4dae-a65a-dcfd2e3f62ad',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'968473'},body:JSON.stringify({sessionId:'968473',location:'cashierPrepayIssue.js:issuePrepayTicket',message:'wallet prepay check',data:{patientId:ctx.pid,amtDue,hasWallet:!!wallet,walletBalance:wallet?parseFloat(wallet.balance):null,walletStatus:wallet?.status||null},timestamp:Date.now(),hypothesisId:'W1',runId:'wallet-fix'})}).catch(()=>{});
      // #endregion
      ...
      if (walletBal < amtDue) {
        ...
        return {
          ok: false,
          error: `${WALLET_INSUFFICIENT_MSG} (balance: ${walletBal.toLocaleString('en-US')} FCFA, required: ${amtDue.toLocaleString('en-US')} FCFA)`,
          status: 402,
        };
      }
      walletHold = wallet;
      ...
      if (lockedBal < amtDue) {
        ...
        return {
          ok: false,
          error: `${WALLET_INSUFFICIENT_MSG} (balance: ${lockedBal.toLocaleString('en-US')} FCFA, required: ${amtDue.toLocaleString('en-US')} FCFA)`,
          status: 402,
        };
      }
      walletHold = locked;

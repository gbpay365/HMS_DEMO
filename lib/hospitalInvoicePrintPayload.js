...
async function buildHospitalInvoicePageData(pool, ticketCode) {
  ...
  const invoiceNumber = await assignInvoiceNumberIfMissing(pool, ticket.id, ticket.facility_id);
  if (invoiceNumber) ticket.invoice_number = invoiceNumber;
  ...
  return {
    ...
    invoice: {
      invoice_number: ticket.invoice_number || ticket.invoice_doc_number || null,

...
const { isCashierPortalCode, resolveCashierLandingUrl } = require('./cashierLanding');
...
function portalUrl(portalCode) {
  ...
  if (isCashierPortalCode(key)) return resolveCashierLandingUrl('/cashier');
  ...
  if (url) return resolveCashierLandingUrl(url);
  if (BUILTIN_HOME_URLS[key]) return resolveCashierLandingUrl(BUILTIN_HOME_URLS[key]);

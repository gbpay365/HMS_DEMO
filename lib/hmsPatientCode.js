...
const PREFIX = String(process.env.HMS_PATIENT_CODE_PREFIX || 'SHG').trim() || 'SHG';
const SUFFIX = String(process.env.HMS_PATIENT_CODE_SUFFIX || 'SOA').trim() || 'SOA';
const PAD = Math.max(4, Math.min(8, parseInt(process.env.HMS_PATIENT_CODE_PAD || '6', 10) || 6));

/** @returns {string} e.g. SHG-000001-SOA */
...
const PG_LOCK_ID = 904231001;

function lockAcquired(row) {
  return row && (row.l === 1 || row.l === true);
}
...
async function allocateNextPatientCodeLocked(conn) {
  const isPg = conn && conn.driver === 'postgres';
  if (isPg) {
    await conn.query('SELECT pg_advisory_lock(?)', [PG_LOCK_ID]);
    try {
      return await allocateNextPatientCode(conn);
    } finally {
      await conn.query('SELECT pg_advisory_unlock(?)', [PG_LOCK_ID]).catch(() => {});
    }
  }

  const [[lockRow]] = await conn.query('SELECT GET_LOCK(?, 15) AS l', [LOCK_NAME]);
  if (!lockAcquired(lockRow)) {

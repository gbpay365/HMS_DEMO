...
async function createNewbornPatientRecord(conn, opts) {
  ...
  const [[mother]] = await conn.query(
    'SELECT id, first_name, last_name, phone, email FROM tbl_patient WHERE id = ? LIMIT 1',
    [motherPatientId]
  );
  ...
  const motherEmail = String(mother.email || '').trim();
  const email = motherEmail || `nb-${String(patientCode).toLowerCase()}@newborn.local`;

  const [ins] = await conn.query(
    `INSERT INTO tbl_patient
     (patient_code, first_name, last_name, gender, dob, phone, email, patient_type,
  ...
  await optionalInTransaction(conn, 'nb_identity', () => refreshPatientIdentityKey(conn, babyId));

  const qrToken = `GBPAY-${babyId}-${Date.now()}`;
  await optionalInTransaction(conn, 'nb_wallet', () =>
    conn.query(

...
/** Sum retail value of pharmacy lines served on a given day. */
async function sumPharmacySalesForDay(pool, day) {
  const d = normalizeDay(day);
  const [[row]] = await pool
    .query(
...
async function loadPendingPharmacyDispense(pool, limit = 150) {
  ...
  const [rows] = await pool
    .query(
      `SELECT oi.id, oi.item_name, oi.quantity, oi.unit_price, oi.service_code, oi.status, oi.paid_at,

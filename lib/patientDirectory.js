...
/** After registration redirect (?patient_id=&q=CODE), backfill code/status if the row exists but search cannot match. */
async function repairHighlightedPatientFromQuery(pool, patientId, code) {
  const pid = parseInt(String(patientId || ''), 10) || 0;
  const norm = String(code || '').trim();
  if (pid < 1 || !norm || !looksLikePatientCode(norm)) return;
  try {
    await pool.query(
      `UPDATE tbl_patient
       SET status = 1,
           patient_code = COALESCE(NULLIF(TRIM(patient_code), ''), ?)
       WHERE id = ?
         AND (
...
async function countActivePatients(pool) {
  ...
  return parseInt(String(countRow?.total ?? 0), 10) || 0;
}
...
async function resolveInsertPatientId(conn, insertResult, patientCode) {
  let pid = parseInt(String(insertResult?.insertId ?? 0), 10) || 0;
  if (pid > 0) return pid;
...
async function finalizePatientRegistration(conn, patientId, patientCode, statusVal) {
  ...
  const [rows] = await conn.query('SELECT * FROM tbl_patient WHERE id = ? LIMIT 1', [pid]).catch(() => [[]]);
  return mapPatientRow(rows?.[0]);
}

...
function patientActiveWhere(alias = '', pool) {
  ...
  if (pool?.driver === 'postgres') {
    // status may be SMALLINT or BOOLEAN — never use IS TRUE (42804 on smallint).
    return `(
      ${a}status IS NULL
...
/** Portable active status literal for INSERT/UPDATE (MySQL tinyint vs PG boolean/smallint). */
function patientActiveStatusValue(poolOrConn) {
  return poolOrConn?.driver === 'postgres' ? 1 : 1;
}
...
async function setPatientActiveWithCode(poolOrConn, patientId, patientCode) {
  ...
  const active = patientActiveStatusValue(poolOrConn);
  if (code) {
    await poolOrConn
      .query(
        `UPDATE tbl_patient
    ...
    await poolOrConn.query('UPDATE tbl_patient SET status = ? WHERE id = ?', [active, pid]).catch(() => {});
  }
}
  ...
  try {
    ...
    const inactive =
      pool?.driver === 'postgres'
        ? `(CAST(status AS INTEGER) = 0 OR LOWER(${st}) IN ('0', 'false', 'f', 'no', 'inactive'))`
...
/** Post-commit safety net — re-activate row and backfill code if registration left it invisible. */

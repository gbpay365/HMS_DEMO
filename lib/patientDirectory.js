...
async function patientRowColumns(pool) {
...
async function selectPatientRows(pool, whereSql, params, orderLimitSql = '') {
...
async function fetchPatientById(pool, id) {
  ...
  const [rows] = await selectPatientRows(pool, 'id = ?', [pid], 'LIMIT 1');
...
async function finalizePatientRegistration(conn, patientId, patientCode, statusVal) {
  ...
  const mapped = await fetchPatientById(conn, pid);

...
(function () {
  ...
  window.openLabRefsModal = function () {
    ...
    fetch('/api/lab/template/' + encodeURIComponent(currentTest.id) + '/refs', { credentials: 'same-origin' })
      ...
      .then(function (j) {
        ...
        if (!window.HmsDiagTpl || !HmsDiagTpl.openRefs) throw new Error('Template modals not ready');
        HmsDiagTpl.openRefs({
          title: L('edit_refs_title', 'Edit reference ranges'),
          ...
          onSave: function (rows) {
            ...
            return fetch('/api/lab/template/' + encodeURIComponent(currentTest.id) + '/refs', {
              method: 'PUT',
              credentials: 'same-origin',
              ...
              .then(function (r) { return r.json(); })
              .then(function (res) {
                if (!res.success) throw new Error((res && res.message) || 'Save failed');
  ...
  window.saveLabRefsModal = function () {
    showToast(L('err_print_module', 'Use the modal Save button.'), 'error');
  };
  ...
  function previewReport() {
    ...
    if (!window.HmsDiagTpl || !HmsDiagTpl.openPreview) {
      showToast(L('err_print_module', 'Report layout module failed to load.'), 'error');
      return;
    }
    HmsDiagTpl.openPreview({
      title: currentTest.name + L('preview_suffix', ' — Preview'),
      html: bodyHtml,
      onPrint: function () { printLabTplReport(); },
      onSave: function () {
        saveLabTplReport();
      },
    });
  ...
  function closeModal() {
    if (window.HmsDiagTpl && HmsDiagTpl.close) HmsDiagTpl.close('preview');
  }

...
/** Maps ACL hub element codes → legacy namespace keys (hub.*). */
export const HUB_CODE_TO_LEGACY_KEY = {
...
export function hubItemLabel(code, fallback, t) {
  const key = HUB_CODE_TO_LEGACY_KEY[String(code || '')];
  if (!key) return fallback || code || '';
  return tFallback(t, key, fallback || code || '', { ns: 'legacy' });
}

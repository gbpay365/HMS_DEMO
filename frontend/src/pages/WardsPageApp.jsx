...
export function WardsPageApp({
  ...
  const canIpdSettle = aclOk(aclUi, 'ward.hero.ipd_settle');
  const canManageBeds = aclOk(aclUi, 'ward.hero.manage_beds');
  ...
  return (
    <div className="page-wrapper hms-surface-module hms-wards-page">
      <div className="content px-4 pb-6 pt-2 sm:px-6">
        <FlashMessages flash={flash} error={error} />

        <SurfaceHero icon="bed" title={t('wards.title')} subtitle={`${t('wards.subtitle')} ${t('wards.hub_link')}`} className="mb-4">
          <div className="hms-staff-hero-toolbar mt-3 flex flex-wrap items-center gap-2">
            ...
            {canIpdSettle ? (
              <a href="/cashier/ipd-settle" className="hms-staff-hero-tab">
                <i className="fa fa-credit-card" aria-hidden="true" />
                {t('wards.ipd_settle')}
              </a>
            ) : null}
            {canManageBeds ? (
              <button type="button" className="hms-staff-hero-tab" onClick={() => setAddBedOpen(true)}>
                <i className="fa fa-bed" aria-hidden="true" />
                {t('wards.manage_beds')}
              </button>
            ) : null}
            <a href="/ipd/handover" className="hms-staff-hero-tab">
              <i className="fa fa-exchange" aria-hidden="true" />
              {t('handover.short_label')}
            </a>
            <a href="/nursing/supply-requests" className="hms-staff-hero-tab">
            ...
            <a href="/wards" className="hms-staff-hero-tab hms-staff-hero-tab--refresh" title={t('wards.refresh')}>
        ...
        <div className="hms-compact-kpi-grid mb-4">
          <StatCard label={t('wards.kpi_total')} value={stats.total || bedCount} tone="brand" icon="bed" size="dense" />
          <StatCard label={t('wards.kpi_avail')} value={stats.avail} tone="brand" icon="check-circle" size="dense" />
          <StatCard label={t('wards.kpi_occupied')} value={stats.occ} tone="warning" icon="user" size="dense" />
          <StatCard label={t('wards.kpi_hk')} value={stats.hk} tone="default" icon="refresh" size="dense" />
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-12">
          <div className="lg:col-span-8">
            {wards.length === 0 ? (
              ...
              wards.map((wardName) => (
                <div key={wardName} className="mb-3 rounded-xl border border-slate-100 bg-white shadow-card">
                  {canManageBeds ? (
                    <button
                      type="button"
                      className="flex w-full items-center justify-between border-b border-slate-100 px-4 py-2.5 text-left hover:bg-slate-50"
                      onClick={() => setManageWard(wardName)}
                    >
                      <div>
                        <h2 className="text-sm font-bold text-ink">{wardName}</h2>
                        <p className="text-[11px] text-slate-500">{t('wards.click_manage')}</p>
                      </div>
          ...
          <div className="lg:col-span-4">
            {pendingBed.length > 0 ? (
              <div className="mb-3 rounded-xl border-2 border-orange-300 bg-orange-50/50 p-3 shadow-card">
                <h3 className="mb-2 text-sm font-bold text-orange-900">
                  {t('wards.awaiting_bed', { count: pendingBed.length })}
                </h3>
                <div className="space-y-2">
                  {pendingBed.map((pb) => (
                    <div key={pb.id} className="flex items-start justify-between gap-2 rounded-lg border border-orange-200 bg-white p-2.5">
                      <div className="min-w-0">
                        <div className="truncate text-sm font-bold">{pb.patient_name}</div>
                        <div className="text-[11px] text-slate-500">{pb.admitting_diagnosis?.slice(0, 60) || t('wards.admission_order')}</div>
                      </div>
                      <button
                        type="button"
            ...
            <div className="sticky top-20 rounded-xl border border-slate-100 bg-white shadow-card">
              <div className="border-b border-slate-100 px-4 py-2.5 text-xs font-bold text-ink">
              ...
              <div className="max-h-[560px] space-y-2 overflow-y-auto p-2.5">
                {activeAdmissions.length === 0 ? (
                  ...
                  activeAdmissions.map((a) => {
                    ...
                    return (
                      <div key={a.id} className="flex gap-2.5 rounded-lg border border-slate-100 p-2.5">
          ...
          <>
            ...
            {addBedOpen ? (
              <WardManageBedsModal
                ...
                wardName={wardNames[0] || t('pages.general_ward')}
                beds={[]}
              />
            ) : null}
          </>
